...
![![](images/clipboard-3013050817.png)shaikh (2016, p.62)](images/inflation/shaikh20161.png)
...
Le déflateur du PIB peut être considéré comme un indice de Paasche. En outre, le dénominateur de l'indice de Paasche est le PIB réel
...
- La PPA représente le nombre d'unité d'une monnaie (par exemple le franc) requis afin d'acheter la même quantité de biens et services qui peut être acheté avec une unité de la monnaie de base/référence (souvent le dollar étatsunien).

## Similarité avec les indices de prix {.smaller}
...
-   PPA: combien coute un même panier de biens et services aux USA et en Suisse

==\> $ppp = \frac{P_{CH}}{P_{USA}} \frac{Q_{CH}}{Q_{USA}}$ ==\> prix d'un même panier dans les deux pays, à quantité égale $Q_{CH} = Q_{USA}$ et donc $ppp = \frac{P_{CH}}{P_{USA}} \frac{Q_{CH}}{Q_{USA}}$ devient simplement $ppp = \frac{P_{CH}}{P_{USA}}$

-   Taux de change réel $e = \frac{EP_{US}}{P_{CH}}$: combien de quantité du panier-type suisse s'échange contre une unité de ce même panier-type aux USA.



## PPA et loi du prix unique {.smaller}

- La théorie de la PPA (loi du prix unique) mise en avant par Cassel (1918) considère la PPA comme un taux de change d'équilibre, ce qui est désormais une vision répandue en macroéconomie. 

- Toutefois, les taux de change nominaux ne convergent que très rarement vers leur PPA car les hypothèses de la loi du prix unique ne sont pas très réalistes (aucune barrière à au commerce, aux flux de capitaux, information parfaite, taux de change uniquement déterminés par la demande de devise pour financer l'échange de biens et service...). 

- Les organisations internationales en charge de calculer la PPA (FMI, Banque Mondiale) mettent en avant elles-même que leur PPA ne peut pas être considéré comme un taux de change ni réel ni d'équilibre.

- Le rapport entre la PPA et le taux de change nominal n'est pas le taux de change réel, mais un indice de niveau des prix (voir slide suivante)

## PPA et indice des prix {.smaller}

- La PPA est donc très similaire aux indices de prix
- Toutefois, la magnitude de la PPA a une dimension monétaire et ne peut donc pas directement être considéré comme un indice des prix (à part si les régions comparées ont la même monnaie par exemple les pays de la zone euro).

- Une mesure du niveau des prix dans un pays donné pour un panier de biens et services est le ratio entre la PPA et le taux de change (#monnaie nationale = 1 unité monnaie de référence (USA)):

$$
Indice\ niveau \ prix_{CH} = \frac{PPA_{CH}}{E_{CH}}  
$$


Par exemple, disons que la PPA pour la Suisse par rapport aux USA est de 1\$ = 0.65chf et que le taux de change est de 1\$ = 0.55chf. Si un touriste en provenance des USA échange 10 dollars pour 5.5chf, il devrait dépenser 6.5chf pour acheter ce qui lui aurait coûté 10 dollars aux USA. Les prix en Suisses sont donc 18% plus élevé par rapport aux USA (0.65/0.55 = 1.18).
...
data %>% 
  ...
  geom_line(aes(x = year, y = ppp_chf_usd/`chf/usd`, color = "indice du niveau des prix en Suisse = PPP/E"))+
  theme_minimal()+
...
data %>% 
  ...
  geom_line()+
  geom_line(aes(x = year, y = log(gdp_nominal/ppp_chf_usd[year == 2020]), color = "PIB PPP ($ US 2020)"))+
  theme_minimal()+
...
data %>% 
  ...
  geom_line(aes(x = year, y = gdp_deflated/ppp_chf_usd[year == 2020], color = "PIB constant 2020, PPA"))+
...
1.  Déflater le PIB par le déflateur du PIB (par exemple en franc constant de 2020) ==\> diviser par le déflateur
2.  Déflater par la PPA ==\> Diviser par la PPA, mais UNIQUEMENT à l'année de base du déflateur du PIB (donc 2020).
...
Les exercices auront le même style que les exercices effectués et vus en cours. Ils seront à réaliser sur R (idéalement avec un document Quarto).
