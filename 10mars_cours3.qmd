---
title: "10 Mars. Comptabilité nationale"
...
## Flux de revenus extérieurs nets (foreign net income FNI) {.smaller}
...
[![Rapport 2026 des tendances conjoncturelles de l'économie suisse publié par le SECO](images/cours3/tendanceconjoncturelle2025.png){width="579"}](https://www.seco.admin.ch/seco/fr/home/Publikationen_Dienstleistungen/Publikationen_und_Formulare/konjunkturtendenz/fruehjar2026.html)
...
## Limites des contributions à la croissance (2) {.smaller}
...
-   Ce genre de calculs de contributions ajustées pour les importations sont possibles, mais seulement avec les tables input-output, voire @baccaro_operationalizing_2024
...
[![](images/dbnomicsexample.png){width="706"}](https://db.nomics.world/AMECO/UQGD?dimensions=%7B%22geo%22%3A%5B%22che%22%5D%7D)
...
data = rdb(ids = "AMECO/UQGD/CHE.1.0.0.0.UQGD")
glimpse(data)
```
...
data = rdb(ids = "AMECO/UQGD/CHE.1.0.0.0.UQGD") %>% 
  ...
  drop_na() %>% # enlever les valeurs manquantes
  rename(
    year = original_period,
    gross_op_surplus = value
  )
...
data %>% 
  ggplot(aes(x = year, y = gross_op_surplus))+
  geom_point()+ 
  geom_line()+
